 ...
 */
    function sku_handler(){
        //SKU信息
        $(".sku_chk").bind("change", function () {
            var hasChk = $(this).is(':checked');
            if(hasChk){
                /*已选中*/
                $(this).parent().siblings('dt').children('b').addClass('sku_title_checked');
            }else{
                /*未选中*/
                //$(this).parent().siblings('dt').children('b').removeClass('sku_title_checked');
                //判断当前行中的checkbox是不是全部都被取消选择，如果是，那就要删除sku_title的class:sku_title_checked
                ...
                $(siblings).each(function (index) {
                    ...
                    if(isChk)
                    {
                        hasSubChecked=hasSubChecked*1+1;
                    }
        ...
        var step = {
            ...
            Creat_Table: function () {
                ...
                var columnIndex = 0;
                var skuTitleLen=skuTtile.length;
                if(skuTitleLen>0)
                {
                    $.each(skuTtile, function (i, item){
                        arrayColumn.push(columnIndex);
                        columnIndex++;
                        arrayTile.push($(item).html().replace("：", ""));
                        var itemName = "Father_Item" + i;
                        var skuTitleChildrens=$(item).parent().siblings('dd').children('input[type=checkbox]:checked');
                        //选中的CHeckBox取值
                        var order = new Array();
                        $(skuTitleChildrens).each(function (){
                            order.push($(this).val());
                        });
                    ...
                    //if (bCheck == true) {
                        ...
                        var itemColumHead = $("<th  style=\"\">价格</th><th style=\"\">库存</th> ");
