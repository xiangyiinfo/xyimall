...
class ProvinceLogic
{
    ...
    function getAllProvince($fields = '*')
    {
        $rs = $this->provinceModel->selectAll('','WHERE is_del=1',null,$fields);
        unset($this->provinceModel);
        return $rs;
    }
    
    function getAll($sql='',$where='WHERE is_del=1',$param=null,$fields = '*')
    {
        $rs = $this->provinceModel->selectAll($sql,$where,$param,$fields);
        unset($this->provinceModel);
        return $rs;
    }
    /**
    ...
    {
        $rs = $this->provinceModel->sql($sql,$param);
        unset($this->provinceModel);
        return $rs;
    }
    ...
    {
        $fields = $this->provinceModel->fields;
        $tmpArray = array();
    ...
    function pageProvinceList( $param)
    {
        $p = 1;
        if (!empty($param['p'])) {
            $p = $param['p'];
        ...
        unset($param['p']);
        $where = ' WHERE 1=1 ';
        $changeResult = $this->provinceModel->getWhereAndParamForPage($param);
        ...
        $rs = $this->provinceModel->page($p, 0, '', $where,$data , $field = '*');
        return $rs;
    }
    ...
    {
        $rs = $this->provinceModel->insert($data);
        return $rs;
    }
    ...
    {
        $rs = $this->provinceModel->_testData;
        $rs['is_del']=1;
        return $rs;
    }
    ...
    {
        $rs = $this->provinceModel->updateAuto($data);
        return $rs;
    }
    ...
    function getById( $id,$fields = '*')
    {
        ...
        $rs = $this->provinceModel->selectOne('','WHERE id=:id',array('id'=>$id),$fields);
        return $rs;
    }
    ...
    {
        $rs = $this->provinceModel->selectOne($sql,$where,$param,$fields);
        return $rs;
    }
}