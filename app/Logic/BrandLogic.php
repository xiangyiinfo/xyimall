...
class BrandLogic
{
    ...
    function getAllBrand($fields = '*')
    {
        $rs = $this->brandModel->selectAll('','WHERE is_del=1',null,$fields);
        unset($this->brandModel);
        return $rs;
    }
    
    function getAll($sql='',$where='WHERE is_del=1',$param=null,$fields = '*')
    {
        $rs = $this->brandModel->selectAll($sql,$where,$param,$fields);
        unset($this->brandModel);
        return $rs;
    }
    /**
    ...
    {
        $rs = $this->brandModel->sql($sql,$param);
        unset($this->brandModel);
        return $rs;
    }
    ...
    function pageBrandList( $param)
    {
        ...
        unset($param['p']);
        $where = ' WHERE 1=1 ';
        $changeResult = $this->brandModel->getWhereAndParamForPage($param);
        ...
        $rs = $this->brandModel->page($p, 0, '', $where,$data , $field = '*');
        return $rs;
    }
    ...
    {
        $rs = $this->brandModel->insert($data);
        return $rs;
    }
    ...
    {
        $rs = $this->brandModel->updateAuto($data);
        return $rs;
    }
    ...
    function getById( $id,$fields = '*')
    {
        ...
        $rs = $this->brandModel->selectOne('','WHERE is_del=1 AND id=:id',array('id'=>$id),$fields);
        return $rs;
    }
    ...
    {
        $rs = $this->brandModel->selectOne($sql,$where,$param,$fields);
        return $rs;
    }

    function getByCateID($cid)
    {
        $rs = $this->sql("select b.id,b.brand_name from brand as b LEFT  JOIN brand_category as bc on b.id=bc.brand_id WHERE bc.category_id=:cid",array('cid'=>$cid));
        return $rs;
    }
}