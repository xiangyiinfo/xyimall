...
class UserLogic
{
    ...
    {
        $rs = $this->userModel->selectAll('','WHERE is_del=1',null,$fields);
    ...
    {
        $rs = $this->userModel->selectAll($sql,$where,$param,$fields);
        unset($this->userModel);
        return $rs;
    }
    /**
    ...
    {
        $rs = $this->userModel->sql($sql,$param);
        unset($this->userModel);
        return $rs;
    }
    ...
    function pageUserList( $param)
    {
        ...
        $changeResult = $this->userModel->getWhereAndParamForPage($param);
        ...
        $rs = $this->userModel->page($p, 0, '', $where,$data , $field = '*');
        return $rs;
    }
    ...
    {
        $rs = $this->userModel->updateAuto($data);
        return $rs;
    }
    ...
    function getById( $id,$fields = '*')
    {
        ...
        $rs = $this->userModel->selectOne('','WHERE is_del=1 AND id=:id',array('id'=>$id),$fields);
        return $rs;
    }
    ...
    {
        $rs = $this->userModel->selectOne($sql,$where,$param,$fields);
        return $rs;
    }
}