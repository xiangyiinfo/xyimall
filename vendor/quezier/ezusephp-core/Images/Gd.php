...
class Gd{
    ...
    public function open($imgname){
        //检测图像文件
        if(!is_file($imgname)) return PubFunc::returnArray(2,false,'不存在的图像文件');
        ...
        if(false === $info || (IMAGETYPE_GIF === $info[2] && empty($info['bits']))){
            return PubFunc::returnArray(2,false,'非法图像文件');
        }
    ...
    public function save($imgname, $type = null, $quality=80,$interlace = true){
        if(empty($this->img)) return PubFunc::returnArray(2,false,'没有可以被保存的图像资源');
    ...
    public function width(){
        if(empty($this->img)) return PubFunc::returnArray(2,false,'没有指定图像资源');
    ...
    public function crop($w, $h, $x = 0, $y = 0, $width = null, $height = null){
        if(empty($this->img)) return PubFunc::returnArray(2,false,'没有可以被裁剪的图像资源');
    ...
    public function thumb($width, $height, $type = Image::IMAGE_THUMB_SCALE){
        if(empty($this->img)) return PubFunc::returnArray(2,false,'没有可以被缩略的图像资源');
        ...
        switch ($type) {
            ...
            default:
                return PubFunc::returnArray(2,false,'不支持的缩略图裁剪类型');
        }
    ...
    public function water($source, $locate = Image::IMAGE_WATER_SOUTHEAST,$alpha=80){
        //资源检测
        if(empty($this->img)) return PubFunc::returnArray(2,false,'没有可以被添加水印的图像资源');
        if(!is_file($source)) return PubFunc::returnArray(2,false,'水印图像不存在');

        //获取水印图像信息
        $info = getimagesize($source);
        if(false === $info || (IMAGETYPE_GIF === $info[2] && empty($info['bits']))){
            return PubFunc::returnArray(2,false,'非法水印文件');
        }
        ...
        switch ($locate) {
            ...
            default:
                ...
                if(is_array($locate)){
                    ...
                    return PubFunc::returnArray(2,false,'不支持的水印位置类型');
                }
        }
    ...
    public function text($text, $font, $size, $color = '#00000000', 
        ...
        //资源检测
        if(empty($this->img)) return PubFunc::returnArray(2,false,'没有可以被写入文字的图像资源');
        if(!is_file($font)) return PubFunc::returnArray(2,false,"不存在的字体文件：{$font}");
        ...
        switch ($locate) {
            ...
            default:
                ...
                if(is_array($locate)){
                    ...
                    return PubFunc::returnArray(2,false,'不支持的文字位置类型');
                }
        }
        ...
        if(is_string($color) && 0 === strpos($color, '#')){
            ...
            return PubFunc::returnArray(2,false,'错误的颜色值');
        }
