...
class Upload {
    ...
    private function setDriver($driver = null, $config = null){
        $driver = $driver ? : ($this->driver       ? : PubFunc::sysConfig('file_upload_type'));
        $config = $config ? : ($this->driverConfig ? : PubFunc::sysConfig('upload_type_config'));
        $class = strpos($driver,'\\')? $driver : 'Core\\Upload\\'.ucfirst(strtolower($driver));
        $this->uploader = new $class($config);
        if(!$this->uploader){
            return PubFunc::returnArray(2,false,"不存在上传驱动：{$class}");
        }
    }
